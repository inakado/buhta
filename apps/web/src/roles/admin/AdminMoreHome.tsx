...
export function AdminMoreHome({
	...
	return (
		<section className="screen-stack director-more-home">
			...
			<section className="director-more-section" aria-labelledby="admin-more-account">
				<h3 className="director-more-section-label" id="admin-more-account">Аккаунт</h3>
				...
				<div className="director-more-section-list">
					{accountRows.map((row) => (
						<AdminMoreRow key={row.id} row={row} />
					))}
				</div>
			</section>
		</section>
...
function AdminMoreRow({ row }: { row: MenuRow }) {
	const Icon = row.icon;
