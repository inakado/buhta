...
describe("shared contracts", () => {
	...
	it("keeps baseline permissions explicit by role", () => {
		...
		expect(permissionsForRole("director")).toContain("client.manage");
		expect(permissionsForRole("director")).toContain("distributor.sale.create");
		expect(permissionsForRole("director")).toContain("distributor.sale.cancel");
	...
	it("validates director analytics contracts", () => {
		...
		expect(DirectorAnalyticsResponseSchema.safeParse({
			...
			production: {
				...
				currentWorkshopProductTotalNetWeightGrams: 2000,
				summary: {
					rawMaterialConsumedQuantity: 10,
