...
const sourceFiles = ["AGENTS.md", ...listDocMarkdownFiles()];
...
for (const sourceFile of sourceFiles) {
  const rawTargets = collectMarkdownTargets(readFile(sourceFile));
  ...
  for (const rawTarget of rawTargets) {
    ...
    const resolvedPath = pathPart ? resolveFromFile(sourceFile, pathPart) : sourceFile;
