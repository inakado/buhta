...
## 7. Production deploy contour

Первый production deploy строится как простой закрытый контур для небольшой команды:

- Ubuntu Server;
- Docker Engine + Docker Compose plugin;
- отдельные production Dockerfiles для `apps/api` и `apps/web`;
- отдельный `deploy/compose.prod.yml`, не смешанный с dev `docker-compose.yml`;
- Caddy как reverse proxy и automatic HTTPS;
- Postgres 18 в Docker named volume на первом этапе;
- GitHub Actions для CI, image build и deploy over SSH;
- GHCR images, тегированные git SHA;
- rollback приложения по предыдущему image tag.

Production CI/CD на `2026-06-11` подготовлен и проверен end-to-end: GitHub Actions выполняет CI, собирает/push images в GHCR и доставляет выбранный git SHA на VPS через SSH.

Production deploy не использует Kubernetes, Terraform, ArgoCD/Flux, blue-green/canary или отдельный staging-сервер на первом этапе. Эти инструменты можно добавить позже только при реальной необходимости.

Текущий MVP-компромисс: API image выполняет обязательный build-step, но runtime пока запускает `tsx src/main.ts`. Причина — `@buhta/shared` сейчас экспортирует TS-исходники для удобного dev-contour; compiled/bundled API runtime вынесен в production-hardening.

Docker images пока не оптимизированы по размеру до production-minimal runtime, но это не блокирует MVP deploy: последний полный deploy прошел успешно. Сжатие images выполнять отдельной задачей, когда появится реальная боль по скорости GHCR push/pull или диску.
