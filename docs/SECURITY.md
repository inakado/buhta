...
- `/direct-accounting/*` защищены единым `direct_accounting.manage`; рабочий UI доступен только Директору, `admin` сохраняет backend support-доступ. Приходы, продажи, затраты, передачи средств и зарплаты изолированы от основных остатков и денег, будущая дата запрещена на backend, каждое изменение и мягкое удаление аудируется.
