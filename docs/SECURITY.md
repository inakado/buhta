...
Текущая policy для дисконта и будущих корректировок: `director` и `admin`. Отмена продажи отличается от управленческой корректировки: ее выполняют операционные роли, которые продают товар, а директор контролирует такие действия через историю/audit. Продажу с распределителя отменяют роли с `distributor.sale.cancel`; курьерскую продажу отменяет курьер по своему контуру через `courier.sale.cancel`. Администратор остается backend-суперпользователем.
