...
- `/direct-accounting/*` защищены единым `direct_accounting.manage`; рабочий UI доступен только Директору, `admin` сохраняет backend support-доступ. Приходы, продажи, затраты и передачи средств изолированы от основных остатков и денег, будущая дата запрещена на backend, каждое изменение и мягкое удаление аудируется.
...
Курьерская продажа разделяет чтение cash, создание и отмену: `GET /courier/cash-balances` требует `courier.cash.read`, `GET /courier/sale-options`, `GET /courier/sales/history` и `POST /courier/sales` требуют `courier.sale.create`, а `POST /courier/sales/:saleId/cancel` требует `courier.sale.cancel`. Курьер видит и меняет только собственный контур; коммерческий руководитель и Директор видят наличные балансы всех курьеров read-only, но не могут оформить или отменить продажу за курьера. Администратор имеет backend-доступ как супер-пользователь: для write-команды с чужим курьером требуется явный `courierUserId`, а history sales backend scope для admin — все courier sales.
