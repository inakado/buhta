...
На `2026-06-12` отслеживаемые в Git workflow обновлены с `actions/checkout@v4` до `actions/checkout@v6` и с `actions/setup-node@v4` до `actions/setup-node@v6.4.0`, чтобы убрать GitHub Actions warning о будущем переходе action runtime с Node 20 на Node 24. Это не меняет runtime Node приложения: проектная версия Node по-прежнему берется из `.node-version`.
