...
- Каталог переведен на compact management surface: текущий раздел справочника сначала показывает список и компактные действия `Добавить` / `Архив`, а создание, редактирование и подтверждение архива открываются в modal dialog, не разрывая ledger rhythm списка.
...
- Главная Директора теперь задает эталонный visual/control language для следующих UI cleanup этапов: плотные read-only панели, табличная типографика, тонкие ledger-разделители, restrained radii, без декоративных карточек, fake statuses и случайных локальных tokens.
- `Остатки Директора` доведены до второго эталонного control surface: embedded segmented tabs, fixed topbar, stock ledger для распределителя и курьеров, compact modal снижения цены, без повторных labels и декоративных карточек.
- `История операций` у Директора визуально приведена к compact director control rhythm: фильтры открываются в modal dialog, верхняя область закреплена, details dialog скрывает technical ids и raw JSON.
...
- `История операций` использует append pagination: `Показать еще` добавляет следующую cursor-страницу ниже текущих строк, а не заменяет список; фильтры сохраняются в browser local storage без quick preset-периодов.
- `История продаж` больше не ограничена последними 10 строками: distributor/courier history читается через paginated endpoints, поддерживает поиск по клиенту/телефону/товару, фильтр `Все / Активные / Отмененные` и отмену любой загруженной активной продажи.
...
| --- | --- | --- | --- | --- | --- |
| UX-001 | P1 | Cross-role profile / auth | Пользователь не может сменить собственный пароль. Сейчас есть только admin reset password и выход. Это общий flow для всех ролей, а не незакрытый директорский экран. | Добавить смену собственного пароля: текущий пароль, новый пароль, повтор, понятные ошибки, сброс локальной сессии при необходимости. | `apps/web/src/app-shell/RoleHomeRouter.tsx`, `apps/api/src/users/users.service.ts` |
...
5. Администратор: пользователи, каталог, клиенты, история операций, настройки.

Директорский набор экранов на 2026-06-08 закрыт для текущего design-system pass. Следующие директорские изменения должны идти либо как cross-role auth/reports hardening (`Сменить пароль`, `Экспорт`), либо как отдельный новый reports/control scope.
