...
| `production.raw_material_intake.create` | Директор, заведующий производством или администратор зафиксировал поступление сырья | нет, audit details содержат id вида сырья, snapshot названия, количество, единицу и комментарий |
| `production.raw_material.correct` | Директор или администратор уменьшил ошибочный или тестовый остаток сырья с обязательной причиной | `raw_material_correction`; audit details содержат id и snapshot названия вида сырья, единицу, количество, причину и остаток до/после |
| `production.packaging_intake.create` | Директор, заведующий производством или администратор зафиксировал поступление тары | нет, audit details содержат id вида тары, snapshot названия, количество, единицу и комментарий |
| `production.product_batch.create` | Директор, заведующий производством или администратор выпустил партию продукции в цеху | нет, audit details содержат шаблон, snapshot названия продукции, сырья, тары, цену, `quantity`, `quantityInput*`, `netWeightGrams`, `totalNetWeightGrams` и расход сырья/тары |
| `production.product_transfer.create` | Директор, заведующий производством или администратор переместил готовую продукцию из цеха на распределитель | `product_transfer`; audit details содержат snapshot продукции, цены, распределителя, `quantity`, `quantityInput*`, `netWeightGrams`, `totalNetWeightGrams` и остатки до/после |
...
| `distributor.stock.correct` | Директор или администратор уменьшил ошибочный или тестовый остаток конкретной priced stock row распределителя | `distributor_stock_correction`; audit details содержат распределитель, партию, продукцию, фактическую цену, `quantity`, `quantityInput*`, `netWeightGrams`, `totalNetWeightGrams`, причину, остаток и стоимость до/после |
