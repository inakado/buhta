...
- `WorkshopProductBalance`, `DistributorProductBalance`, `ProductTransfer` и backfill workshop balances для существующих партий;
- выпуск партии создает workshop product balance в той же transaction;
- summary и экран “Продукция в цеху” читают доступный остаток из workshop balances, а не из `ProductBatch.status`;
- `POST /production/product-transfers` транзакционно уменьшает остаток в цеху, увеличивает остаток распределителя, пишет `operation`, `audit_log` и typed `product_transfer`;
- `GET /production/workshop-product-balances` и `GET /production/transfer-options` добавлены для production UI;
- mobile production screen получил действие `На распределитель` с online-only submit;
...
- `corepack pnpm --filter @buhta/api exec vitest run test/production-controller.test.ts test/production-db.integration.test.ts test/policy.test.ts`;
...
Примечание: API integration tests требуют доступ к локальному Postgres на `localhost:5433`; в sandbox они могут падать на доступе к БД, поэтому targeted integration run и полный `pnpm test` выполнялись вне sandbox после `prisma migrate deploy`. Первый `pnpm build` в sandbox упал на Turbopack `binding to a port / Operation not permitted`, повтор вне sandbox прошел успешно.
