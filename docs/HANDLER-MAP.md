...
| `POST /production/product-transfers` | `ProductionController.createProductTransfer` | transfer finished product from workshop to distributor with conditional workshop decrement and distributor balance increment | `production.manage` через `RequirePermission` + `PolicyGuard` | `operation` + `audit_log` с `production.product_transfer.create`, typed `product_transfer` | `apps/api/test/production-controller.test.ts`, `apps/api/test/production-db.integration.test.ts`, `apps/web/app/page.test.tsx` |
| `GET /distributor/inventory` | `DistributorController.inventory` | read non-zero distributor product balances with total stock value summary | `distributor.stock.read` через `RequirePermission` + `PolicyGuard` | нет | `apps/api/test/distributor-controller.test.ts`, `apps/api/test/distributor-inventory-db.integration.test.ts`, `apps/api/test/policy.test.ts`, `apps/web/app/page.test.tsx` |
| `GET /distributor/sale-options` | `DistributorController.saleOptions` | read active distributor stock rows available for distributor sale | `distributor.sale.create` через `RequirePermission` + `PolicyGuard` | нет | `apps/api/test/distributor-controller.test.ts`, `apps/api/test/distributor-sales-db.integration.test.ts`, `apps/api/test/policy.test.ts`, `apps/web/app/page.test.tsx` |
| `GET /distributor/cash-balances` | `DistributorController.cashBalances` | read distributor cash balance projection, returning zero for distributors without cash row | `distributor.cash.read` через `RequirePermission` + `PolicyGuard` | нет | `apps/api/test/distributor-controller.test.ts`, `apps/api/test/distributor-sales-db.integration.test.ts`, `apps/api/test/policy.test.ts`, `apps/web/app/page.test.tsx` |
| `POST /distributor/sales` | `DistributorController.createSale` | create distributor sale by `distributorProductBalanceId`, decrement stock and increment cash only for cash payment | `distributor.sale.create` через `RequirePermission` + `PolicyGuard` | `operation` + `audit_log` с `distributor.sale.create`, typed `distributor_sale` | `apps/api/test/distributor-controller.test.ts`, `apps/api/test/distributor-sales-db.integration.test.ts`, `apps/api/test/policy.test.ts`, `apps/web/app/page.test.tsx` |
