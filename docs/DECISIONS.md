...
## DEC-09 — Net weight input with unit-based stock

Статус: `Accepted`

Решение:

- готовая продукция продолжает учитываться целыми единицами в таре;
- `quantity` остается каноническим количеством для партий, остатков, продаж, дисконтов, загрузок и возвратов;
- `ProductTemplate.netWeightGrams` хранит массу нетто одной единицы продукции в граммах;
- `ProductBatch.netWeightGrams` snapshot-ит массу нетто на момент выпуска партии;
- товарные write-команды могут принимать `quantityInput` в режиме кг или штук;
- backend пересчитывает ввод в кг в целые штуки по snapshot массы нетто и запрещает некратную массу без округления;
- `priceCents` и `unitPriceCents` остаются ценой за штуку, не за кг;
- UI по умолчанию открывает товарные операции в режиме кг, но показывает и учетные штуки.

Причина:

- в реальной работе производства, распределителя, курьеров и продаж пользователи говорят о массе икры в кг;
- покупатели тоже заказывают в кг, поэтому kg-first ввод снижает когнитивную нагрузку;
- складские инварианты проще и надежнее защищать целыми физическими единицами в таре;
- цена за банку уже является действующим бизнес-правилом, и переход на `₽/кг` не требуется для текущей v1.
